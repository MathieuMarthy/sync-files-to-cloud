...
class GDriveCloudDAO(CloudDAO):
    ...
    def upload_files(
        ...
        try:
            ...
            for file in files:
                target_folder_id = self._determine_target_folder(
                    file, remote_folder, folder_id, local_base_path
                )
                self._upload_single_file(file, target_folder_id)
        except ServerNotFoundError:
            raise NoInternet(
                "Don't have access to internet or the cloud provider api is down"
            )
    ...
    def _determine_target_folder(
        ...
        if local_base_path and file.is_relative_to(local_base_path):
            ...
            if relative_path.parent != Path("."):
                ...
                target_folder_path = (
                    f"{remote_folder.rstrip('/')}/{relative_path.parent.as_posix()}"
                )
    ...
    def _find_existing_file(self, q_name: str, target_folder_id: str) -> str | None:
        ...
        query = (
            f"name = '{q_name}' and '{target_folder_id}' in parents and trashed = false"
        )
        results = (
            self.gdrive_service.files()
            .list(q=query, spaces="drive", fields="files(id, name, md5Checksum)")
            .execute()
        )
    ...
    def _update_file_if_changed(self, file: Path, name: str, existing_file_id: str):
        ...
        remote_file = (
            self.gdrive_service.files()
            .get(fileId=existing_file_id, fields="md5Checksum")
            .execute()
        )
        ...
        if remote_md5 == local_md5:
            logger.debug(f"File '{name}' is already up to date, skipping upload")
        else:
            ...
            logger.debug(
                f"File '{name}' has been updated with ID: {updated_file['id']}"
            )
    ...
    def _create_new_file(self, file: Path, name: str, target_folder_id: str):
        ...
        logger.debug(f"File '{name}' uploaded with ID: {uploaded_file['id']}")
    ...
    def _get_or_create_folder(self, folder_path: str) -> str:
        ...
        for i, folder_name in enumerate(folder_names):
            ...
            if items:
                ...
                logger.debug(
                    f"Found existing folder '{folder_name}' with ID: {parent_id}"
                )
            else:
                ...
                parent_id = folder["id"]
                logger.debug(f"Created folder '{folder_name}' with ID: {parent_id}")
    ...
    def init_connection(self, can_open_connection_page: bool = False, folder=None):
        ...
        if os.path.exists(utils.path(TOKEN_PATH)):
            creds = Credentials.from_authorized_user_file(
                utils.path(TOKEN_PATH), SCOPES
            )

        # If there are no (valid) credentials available, let the user log in.
        if not creds or not creds.valid:
            if creds and creds.expired and creds.refresh_token:
                logger.debug("GDrive: token expired, refreshing the token")
                creds.refresh(Request())
            elif can_open_connection_page:
                logger.debug("GDrive: no valid token found, starting the login flow")

                if not os.path.exists(utils.path(CREDENTIALS_PATH)):
                    raise NoCredentialFileException(
                        f"GDrive: missing credentials file for authentication. Please provide the file at {CREDENTIALS_PATH}"
                    )
                ...
                raise AuthentificationRequiredException(
                    "GDrive: need to authenticate in the browser"
                )
        ...
        logger.debug("GDrive: connection established")
